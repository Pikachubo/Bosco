...
Sourcecode https://github.com/pepesir/pikachubo
...
let keynye = m.key
let c = pikachubo.chats.get(keynye.remoteJid)
let a = c.messages.dict[`${keynye.id}|${keynye.fromMe ? 1: 0}`]
let contennye = pikachubo.generateForwardMessageContent(a, false)
} catch {
}
}


module.exports = pikachubo = async (pikachubo, mek) => {
	try {
        ...
        const botNumber = pikachubo.user.jid
        const ownerNumber = setting.ownerNumber
		const ownerName = setting.ownerName
		const botName = setting.botName
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? pikachubo.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const totalchat = await pikachubo.chats.all()
		...
		const groupMetadata = isGroup ? await pikachubo.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? pikachubo.user.jid : pikachubo.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? pikachubo.user.name : conts.notify || conts.vname || conts.name || '-'
		...
		const catl = (teks) => {
             res = pikachubo.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": teks, "footerText": "*Pepe Ser*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pikachubo.relayWAMessage(res)
        }
       const catlo = (teks) => {
             res = pikachubo.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": teks, "footerText": "Made With Pepe", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:ftroli})
             pikachubo.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pikachubo.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/BzhyWkAEU0t8oVl3s8p94m', "groupName": `pikachubo Family`, "footerText": "*Pepe Ser*", "jpegThumbnail": dfrply, "caption": teks}}, {quoted:fvideo})
            pikachubo.relayWAMessage(grup)
        }
        try {
		pporang = await pikachubo.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
        ...
        const reply = (teks) => {
	      pikachubo.sendMessage(from, teks, text, { thumbnail: denis, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: false, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : PEPE SIR${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}},})
        }
        const sendMess = (hehe, teks) => {
           pikachubo.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? pikachubo.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.jpg')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : pikachubo.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.jpg')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           pikachubo.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return pikachubo.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/wpmobile.jpg')})
        }
        
        const fakestatus = (teks) => {
            return pikachubo.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
        ...
        const fakethumb = (teks, yes) => {
            return pikachubo.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpg'),quoted:mek,caption:yes})
        }
      ...
      const fgclink = (teks) => {
            pikachubo.sendMessage(from, teks, text, {
                quoted: {
	                key: {
		               fromMe: true,
...
kma = gam1
mhan = await pikachubo.prepareMessage(from, kma, location)
...
headerType: 6
/}
pikachubo.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
/}*/
//Button ocument
const Sendbutdocument = async(id, text1, desc1, file1, doc1, but = [], options = {}) => {
media = file1
kma = doc1
mhan = await pikachubo.prepareMessage(from, media, document, kma)
const buttonMessages = {
...
const ftrol = {
	key : {
       ...
       const kick = function(from, orangnya){
	       ...
	       pikachubo.groupRemove(from, [i])
        }
        }
       const kickMember = async(id, target = []) => {
           let group = await pikachubo.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = pikachubo.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await pikachubo.groupRemove(from, [i])
        } else {
           await pikachubo.sendMessage(id, "Not Premited!", "conversation")
        }
    }
}
// AUTO
			for (let anji of setik){
				if (budy === anji){
					...
					pikachubo.sendMessage(from, result, sticker, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: from } : {})}, message: { orderMessage: { itemCount: 2021, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${pushname}`, orderTitle: `${pushname}`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					pikachubo.sendMessage(from, result, audio, { quoted: mek, mimetype: 'audio/mp4', duration: 1, ptt: true, contextInfo: { forwardingScore: 0, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					pikachubo.sendMessage(from, result, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${fake}`, orderTitle: `MADE BY DENIS`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
			for (let anje of videonye){
				if (budy === anje){
					result = fs.readFileSync(`./media/video/${anje}.mp4`)
					pikachubo.sendMessage(from, result, video, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}, mimetype: 'video/mp4' })
					}
			}
       const add = function(from, orangnya){
	       pikachubo.groupAdd(from, orangnya)
}
      const sendBug = async(target, teks) => {
           if (!teks) teks = '.'
           await pikachubo.relayWAMessage(pikachubo.
           prepareMessageFromContent(target, pikachubo.
           prepareDisappearingMessageSettingContent(0),
           {}),{waitForAck:true})
           pikachubo.sendMessage(target, teks, 'conversation')
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       pikachubo.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: mek})
}
      const hideTag = async function(from, text){
	       let anu = await pikachubo.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       pikachubo.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Nakano.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           pikachubo.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:mek})
           fs.unlinkSync(asw)
});
});
});
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           pikachubo.sendMessage(to, media, type, {quoted: mek, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : PEPE SIR${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
            ...
            const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        pikachubo.sendMessage(to, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(filess)
                        fs.unlinkSync(asw)
                    });
                });
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       pikachubo.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       pikachubo.sendMessage(from, hasil, type, options).catch(e => {
	       pikachubo.sendMessage(from, { url : link }, type, options).catch(e => {
          ...
          let authorname = pikachubo.contacts[from] != undefined ? pikachubo.contacts[from].vname || pikachubo.contacts[from].notify : undefined	
...
// Anti link
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				pikachubo.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link terdeteksi, Auto kick!')
			    pikachubo.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
        if (isGroup && isAntiLink && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match("https:\\chat.whatsapp.com")) {
                reply(`?? *GROUP LINK DETECTOR* 🚧\n\n_To Any Links Send This Group You Will Kicked_`)
                pikachubo.groupRemove(from, [sender])
            }
        }
       ...
       if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
        ...
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
		    ...
		    pikachubo.groupMakeAdmin(from, [picknya])
		    ...
		    pikachubo.groupDemoteAdmin(from, [dicknya])
		    return reply(`*Dᴇᴍᴏᴛᴇᴅ*`)
			}
         
        // Button Cmd 
           if (responseButton === 'open') {
	       pikachubo.sendMessage(from, `*Gʀᴏᴜᴘ Oᴘᴇɴᴅ Bʏ Aᴅᴍɪɴ*`, MessageType.text, {quoted: ftext})
		   pikachubo.groupSettingChange(from, GroupSettingChange.messageSend, false)
		   } else if (responseButton === 'close') {
	       await pikachubo.groupSettingChange(from, GroupSettingChange.messageSend, true)
	       pikachubo.sendMessage(from, `*Gʀᴏᴜᴘ Cʟᴏsᴇᴅ Bʏ Aᴅᴍɪɴ*`, MessageType.text, {quoted: ftext})
           }
           if (responseButton === 'on'){
           await pikachubo.toggleDisappearingMessages(from, WA_DEFAULT_EPHEMERAL)
           } else if (responseButton === 'off'){
           await pikachubo.toggleDisappearingMessages(from, 0)
           }
        ...
        if (isCmd && isGroup)
			...
			const vname_ = pikachubo.contacts[i] != undefined ? pikachubo.contacts[i].vname || pikachubo.contacts[i].notify : undefined
		     ini_list.push({
			 "displayName": 'Owner pikachubo',
			 "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${pikachubo.user.name}`}\nORG: Pepe Ser;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Its me Pepe\nEND:VCARD`
			  })
			  }
			 hehe = await pikachubo.sendMessage(from, {
        ...
        case 'pikachubo':
        case 'cmd':
        groups = pikachubo.chats.array.filter(v => v.jid.endsWith('g.us'))
        privat = pikachubo.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
        totalChat = await pikachubo.chats.all()
        pikachubo1 = await pikachubo.prepareMessage(from, denis, location, {thumbnail: denis})
        pikachubo2 = pikachubo1.message["ephemeralMessage"] ? pikachubo1.message.ephemeralMessage : pikachubo1
        timestampe = speed();
...
wa.me/${owner}`
       osk = pikachubo.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1000, "message": `${rules1}`, "footerText": "hehe", "thumbnail": denis, "surface": 'CATALOG'}}, {quoted: mek})
            pikachubo.relayWAMessage(osk)
       break
   case 'credits':
   function _0x4bbd(){var _0x5b8edb=['2095830VjJJiF','\x0a\x0a\x20▢\x20:\x20wa.me/','\x0a\x0a\x20','5267880lROrnj','CATALOG','33609wAGVBo','0','prepareMessageFromContent','relayWAMessage','4972079yWqsym','126495MscqCV','472Lglbxe','\x0a\x0a\x20\x20\x20[\x20𝐵𝑜𝑠𝑐𝑜\x20𝐶𝑟𝑒𝑑𝑖𝑡𝑠\x20𝐵𝑦\x20]\x20:\x0a\x0a\x20\x20\x20\x0a\x0a\x20▢\x20:\x20wa.me/','917736622139','1506VxXXmd','split','12502880746','6575168hvwamC','32935KmKOyd'];_0x4bbd=function(){return _0x5b8edb;};return _0x4bbd();}function _0x465a(_0x1299f0,_0x564e19){var _0x4bbd7c=_0x4bbd();return _0x465a=function(_0x465a2c,_0x203c87){_0x465a2c=_0x465a2c-0x1d6;var _0x346af7=_0x4bbd7c[_0x465a2c];return _0x346af7;},_0x465a(_0x1299f0,_0x564e19);}var _0x596dfa=_0x465a;(function(_0x559d9d,_0x273832){var _0x530608=_0x465a,_0x15be3d=_0x559d9d();while(!![]){try{var _0x581c1b=parseInt(_0x530608(0x1d9))/0x1+-parseInt(_0x530608(0x1e2))/0x2+parseInt(_0x530608(0x1e7))/0x3*(parseInt(_0x530608(0x1da))/0x4)+-parseInt(_0x530608(0x1e1))/0x5*(parseInt(_0x530608(0x1dd))/0x6)+parseInt(_0x530608(0x1d8))/0x7+parseInt(_0x530608(0x1e0))/0x8+parseInt(_0x530608(0x1e5))/0x9;if(_0x581c1b===_0x273832)break;else _0x15be3d['push'](_0x15be3d['shift']());}catch(_0x5572dc){_0x15be3d['push'](_0x15be3d['shift']());}}}(_0x4bbd,0xd31c6),dtod=_0x596dfa(0x1df),dtod1='0',dtod2=_0x596dfa(0x1dc),dtod3=_0x596dfa(0x1e8),dtod4='0');var v=_0x596dfa(0x1db)+dtod1[_0x596dfa(0x1de)]('@')[0x0]+_0x596dfa(0x1e3)+dtod[_0x596dfa(0x1de)]('@')[0x0]+_0x596dfa(0x1e3)+dtod2['split']('@')[0x0]+'\x0a\x0a\x20▢\x20:\x20wa.me/'+dtod3[_0x596dfa(0x1de)]('@')[0x0]+_0x596dfa(0x1e3)+dtod4[_0x596dfa(0x1de)]('@')[0x0]+_0x596dfa(0x1e4);credit=pikachubo[_0x596dfa(0x1d6)](from,{'orderMessage':{'itemCount':0x3e8,'message':''+v,'footerText':'hehe','thumbnail':denis,'surface':_0x596dfa(0x1e6)}},{'quoted':mek}),pikachubo[_0x596dfa(0x1d7)](credit);
        break
//------------------< Sticker Cmd >-------------------
...
//------------------< Bot Owner >-------------------
        ...
        case 'clearall':
					anu = await pikachubo.chats.all()
					pikachubo.setMaxListeners(25)
					for (let _ of anu) {
					 pikachubo.deleteMessage(_.jid)
					}
					reply('*done*')
					break
         case 'setprefix':
				prefix = args.join(' ')
				pikachubo.sendMessage(from, `*Succes Changing Prefix : ${prefix}*`, text, {quoted: ftoko, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
				break
         case 'getquoted':
       ...
       case 'group':
        rows = [
          ...
          pikachubo.sendMessage(from, button, MessageType.listMessage, {quoted: ftroli})
                    break
         case 'online':
          ...
          case 'tobc':
					...
					buff = await pikachubo.downloadMediaMessage(encmedia)
					for (let _ of anu) {
					pikachubo.sendMessage(_.jid, buff, gif, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 1000, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `𝑬𝑽𝑬𝑹𝒀𝑻𝑯𝑰𝑵𝑮\n𝑾𝑰𝑳𝑳 𝑩𝑬\n😎𝑶𝑲😎`, orderTitle: `ʙʀᴏᴀᴅᴄᴀsᴛ`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 22, isForwarded: true}})
					}
					} else {
...
break
        ...
        if (
          ...
          boij =
            isQuotedImage || isQuotedSticker
              ...
              : mek;
          delb = await pikachubo.downloadMediaMessage(boij);
     ...
     case 'searchmsg':  //by ANU TEAM
             ...
             reply(mess.wait)
             cok = await pikachubo.searchMessages(`${ve}`, from, batas,1) 
             ...
             pikachubo.sendMessage(from, `*Found..!*`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
             } catch (e) {
     ...
     case 'seenby':
             if(!isGroup) return reply(mess.only.group)
             try {
             infom = await pikachubo.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
             tagg = []
             teks = `*• Read by :*\n\n`
      ...
      case 'leave':
       ...
       case 'here':                
             ...
             let online = [...Object.keys(pikachubo.chats.get(ido).presences), pikachubo.user.jid]
             pikachubo.sendMessage(from, '*List Online:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      case 'join': 
             ...
             fak = pikachubo.query({ json: ['action', 'invite', link],
             expect200: true })
             reply('*Successfully Entered Group*')
             break
      case 'readall':
					if (!mek.key.fromMe) return reply('```OWNER ONLY```')
					var chats = await pikachubo.chats.all()
                    chats.map( async ({ jid }) => {
                          await pikachubo.chatRead(jid)
                    })
					rdl = `*Successfully read ${chats.length} Chat !*`
					await pikachubo.sendMessage(from, rdl, MessageType.text, {quoted: ftroli})
      ...
      case 'imgtourl':
       case 'img2url':
               ...
               var mediahe = await  pikachubo.downloadAndSaveMediaMessage(encmediahe)       
               imgbb('39d895963468b814fad0514bd28787e2', mediahe)
              .then(data => {
               var caps = `*IMAGE TO URL*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(mediahe)
               pikachubo.sendMessage(from, ibb, image, { quoted: mek, caption: caps})
})
              .catch(err => {
               throw err
})
...
*Sɪᴢᴇ :* ${res[0].size}`
              ...
              yt1 = await pikachubo.prepareMessage(from, fs.readFileSync(`./ytmp.jpeg`), location, {thumbnail: fs.readFileSync(`./ytmp.jpeg`),})
              ...
              prep = await pikachubo.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftroli})
              pikachubo.relayWAMessage(prep)
              fs.unlinkSync(`./ytmp.jpeg`)
              break
      case 'rall':
				const readallid = await pikachubo.chats.all()
			pikachubo.setMaxListeners(25)
				for (let xyz of readallid) {
					await pikachubo.chatRead(xyz.jid)
				}
		      pikachubo.sendMessage(from, `Sukses!`, text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "mimetype": "image/jpeg", "caption": "Berhasil membaca semua chat!", 'jpegThumbnail': fs.readFileSync('./ds.jpg')}}}})
              break
      case 'shutdown':
      ...
      case 'leaveall':
             if (!isOwner) return  
             let totalgroup = pikachubo.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
             for (let id of totalgroup) {
             sendMess(id, 'Byee', null)
             await sleep(3000)
             pikachubo.groupLeave(id)
}
             break
       case 'culik':
              if (!isOwner) return
              ...
              pikachubo.groupAdd(args[0], pantek)
              break
...
//------------------< Editing/Audio/Video >-------------------
               ...
               case 'secvn':
					...
					media = await pikachubo.downloadAndSaveMediaMessage(encmedia)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(media)
					pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
					fs.unlinkSync(media)
				    break
				case 'secvideo':
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					cokmatane = Number(args[0])
					hah = fs.readFileSync(media)
					pikachubo.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
					fs.unlinkSync(media)
				    break
               case 'voice':
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					pikachubo.sendMessage(from, topt, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
					})
					break
               case 'take':
              case 'colong':
			        ...
			        require('./lib/fetch.js').modStick(media, pikachubo, mek, from)
			        break
             case 'fastvid':
		            ...
		            pikachubo.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
		            fs.unlinkSync(ran)
		            })
		            break
		    case 'slowvid':
		    ...
		    case 'reversevid':
             ...
             case 'nightcore':
					...
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						...
						pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: ftoko,duration:11})
						fs.unlinkSync(ran)
					   })
				       break 
             ...
             case 'baby':
                   if (!isQuotedAudio) return fgclink('reply to audio')
					...
					if (err) return reply('Error!')
					hah = fs.readFileSync(ran)
					pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, quoted: mek})
					fs.unlinkSync(ran)
			        })
				     break
           case 'cm':
					...
					exec(`ffmpeg -i ${media} "origin(rgb24).png" -c:v libx264 -preset placebo -qp 0 -x264-params "keyint=15:no-deblock=1" -pix_fmt yuv444p10le -sws_flags spline+accurate_rnd+full_chroma_int -vf "colormatrix=bt470bg:bt709" -color_range 1 -colorspace 1 -color_primaries 1 -color_trc 1 "colormatrix_yuv444p10le.avi" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						pikachubo.sendMessage(from, hah, video, { mimetype: 'video/mp4', quoted: ftoko })
					})
					break
           case 'fast':
                     if (!isQuotedAudio) return fgclink('reply to audio')
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=1.3,asetrate=43000" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: ftext})
						fs.unlinkSync(ran)
					})
					break
             case 'gemes':
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=1.0,asetrate=50000" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
					pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: fgc})
						fs.unlinkSync(ran)
					})
					break
            case 'balik':
            case 'reverse':
	              ...
	              mediau = await pikachubo.downloadAndSaveMediaMessage(encmediau)
	              ran = getRandom('.mp3')
	              exec(`ffmpeg -i ${mediau} -filter_complex "areverse" ${ran}`, (err, stderr, stdout) => {
                  fs.unlinkSync(mediau)
                  if (err) return reply('Error!')
                  hah = fs.readFileSync(ran)
                  pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, duration: 359996400, quoted:mek})
                  fs.unlinkSync(ran)
	              })
                  break
            case 'bass':
            case 'volume':
             ...
             case 'slow':
				...
				case 'blub':
					...
					if (err) return reply('Error!')
					hah = fs.readFileSync(ran)
					pikachubo.sendMessage(from, hah, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek })
					fs.unlinkSync(ran)
					 })
				    break
				case 'fat':
      ...
      case 'tomp4':
            ...
            owgi = await pikachubo.downloadAndSaveMediaMessage(ger)
        ...
        case 'mp3':
					...
					mediad = await pikachubo.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						pikachubo.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 4, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
      case 'kick':
...
pikachubo.groupRemove(from, mems_ids)
} else {
pikachubo.groupRemove(from, [entah[0]])
}
} else {
entah = mek.message.extendedTextMessage.contextInfo.participant
pikachubo.groupRemove(from, [entah])
}
break
case 'add':
      if (!isOwner && !isGroupAdmins) return reply('*Admin Group Only*')
...
pikachubo.groupAdd(from, memu_ido)
} else {
pikachubo.groupAdd(from, [entah[0]])
}
} else {
entah = mek.message.extendedTextMessage.contextInfo.participant
pikachubo.groupAdd(from, [entah])
}
break
case 'promote':
...
pikachubo.groupMakeAdmin(from, memi_idi)
} else {
pikachubo.groupMakeAdmin(from, [entah[0]])
}
} else {
entah = mek.message.extendedTextMessage.contextInfo.participant
pikachubo.groupMakeAdmin(from, [entah])
}
break
case 'demote':
      if (!isOwner && !isGroupAdmins) return reply('*Admin Group Only*')
...
pikachubo.groupDemoteAdmin(from, memk_idk)
} else {
pikachubo.groupDemoteAdmin(from, [entah[0]])
}
} else {
entah = mek.message.extendedTextMessage.contextInfo.participant
pikachubo.groupDemoteAdmin(from, [entah])
}
break
      ...
      case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await pikachubo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikachubo.sendMessage(_.jid, buff, image, { caption: `${body.slice(4)}`})
						}
						...
						buff = await pikachubo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikachubo.sendMessage(_.jid, buff, video, { caption: `${body.slice(4)}`})
						}
						reply(`*Broadcast success* ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						 encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await pikachubo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikachubo.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`*Broadcast success* ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
break
       case 'getdeskgc':
				if (!isGroup) return reply(mess.group)
				anu = from
			   metadete = await pikachubo.groupMetadata(anu)
				pikachubo.sendMessage(from, metadete.desc, MessageType.text, {quoted:fgif})
				  break
					case 'getbio':
	                  var yy = mek.message.extendedTextMessage.contextInfo.participant
                       var p = await pikachubo.getStatus(`${yy}`, MessageType.text, {quoted: mek})
                        reply(p.status)
                         if (p.status == 401) {
           ...
           case 'chat':
            ...
            var org = pc.split("|")[1];
            pikachubo.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`*Success in sending chat* :\n${org},@${nomor}`)
            break
                    case 'getname':
                      var ambl = mek.message.extendedTextMessage.contextInfo.participant
                      const sname = pikachubo.contacts[ambl] != undefined ? pikachubo.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : pikachubo.contacts[ambl].notify || pikachubo.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
                       reply(sname)
                        break
      case 'toimg':
      case 'photo':
              ...
              pikachubo.sendMessage(from, buffer, image, {quoted: mek, thumbnail:denis, caption: 'By pikachubo'})
              fs.unlinkSync(ran)
})
              break
//------------------< Public/Self >-------------------
...
//------------------< Downloader/And Search Media >-------------------
       ...
       case 'igstory': 
             ...
             .then(async result => {
              ...
              pikachubo.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
              } else {
              let link = await getBuffer(i.url)
              pikachubo.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
              }
              }
              });
              break
      ...
      pikachubo.sendMessage(from, hemmo, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
      break
      case 'readmore':
      case 'more':
        ...
        case 'cekapikey':
               ...
               pikachubo.sendMessage(from, teks, text, {quoted: mek})
               break
       case 'pinterest':
         case 'pin':
              ...
              imageMsg = ( await pikachubo.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              ...
              prep = await pikachubo.prepareMessageFromContent(from,{buttonsMessage},{})
              pikachubo.relayWAMessage(prep)
...
🖇️ Link : ${i.url}\n`
       ...
       case 'tourl':
               ...
               owgi = await pikachubo.downloadMediaMessage(boij)
       ...
       case 'telestiker':
              ...
              pikachubo.sendMessage(from, ini_buffer, sticker, {})
}
              break
       case 'ghsearch': 
       case 'githubsearch': 
...
`.trim()
     ...
     case 'googleimage':
      ...
      case 'tiktokdl':
              ...
              prep = await pikachubo.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              pikachubo.relayWAMessage(prep)
              fs.unlinkSync(`./${sender}.jpeg`)
              break
      case 'ttnowm': 
      ...
      case 'tiktokaudio':
             ...
             pikachubo.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
             break
      case 'google':
              if (!q) return reply(mess.wrongFormat)
              ...
              vars +=  `\n━━━━━━━━━━━━━━━━━\n\n*Title:* ${results[i].title}\n\n*Description:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n`
               ...
               pikachubo.sendMessage(from, ss, image, {caption: vars, quoted : mek, thumbnail: Buffer.alloc(0) })
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
               })
...
`
              ...
              pikachubo.sendMessage(from, ini_buffer, image, { quoted: mek, caption: teks })
              } catch {
     ...
     case 'buttons3': 
             ...
             pikachubo.sendMessage(from, ini_video, video, { quoted: mek })
             break
      case 'buttons4': 
             if (!q) return reply('*Where is the link?*')
             if (!q.includes('tiktok')) return reply(mess.error.Iv)
             data = await getBuffer(`https://api.lolhuman.xyz/api/tiktokmusic?apikey=${lolkey}&url=${args[0]}`)
             pikachubo.sendMessage(from, data, audio, { quoted: mek })
             break
      case 'alive':
              pikachubo1 = await pikachubo.prepareMessage(from, denis, location, {thumbnail: denis})
              pikachubo2 = pikachubo1.message["ephemeralMessage"] ? pikachubo1.message.ephemeralMessage : pikachubo1
...
*Wᴀ ᴠᴇʀsɪᴏɴ :* ${pikachubo.user.phone.wa_version}
*Os ᴠᴇʀsɪᴏɴ :* ${pikachubo.user.phone.os_version}
*Dᴇᴠɪᴄᴇ Mᴀɴᴜғᴀᴄᴛᴜʀᴇ :* ${pikachubo.user.phone.device_manufacturer}
*Dᴇᴠɪᴄᴇ Mᴏᴅᴇʟ :* ${pikachubo.user.phone.device_model}
`
    menubutton = [{buttonId:`${prefix}credits`,buttonText:{displayText:'CREDITS'},type:1}
]
 menumessage = { contentText: `${teks}`, footerText: `Sᴜʙsᴄʀɪʙᴇ Yᴛ Pᴇᴘᴇ Sɪʀ`, buttons: menubutton, headerType: 6, locationMessage: pikachubo2.message.locationMessage}
 pikachubo.sendMessage(from, menumessage, MessageType.buttonsMessage)
             break  
      case 'buttons5':
 ...
 //------------------< Extra >-------------------
       ...
       case 'ss':
        ...
        case 'ocr': 
				    ...
				    const media = await pikachubo.downloadAndSaveMediaMessage(encmedia)
					reply(mess.wait)
...
*Pʟᴇᴀsᴇ Wᴀɪᴛ Sᴏɴɢ Lᴏᴀᴅɪɴɢ....*`
            ...
            file = await pikachubo.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await pikachubo.groupMetadata(from)
            ...
            pikachubo.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikachubo.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikachubo.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikachubo.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikachubo.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await pikachubo.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
        ...
        break
         case 'forward':
           if (!isOwner) return reply(mess.only.owner)
	        pikachubo.sendMessage(from, `${body.slice(8)}`, MessageType.text, { sendEphemeral: true, thumbnail: fs.readFileSync('./ds.jpg', 'base64'), contextInfo: { forwardingScore: 2, isForwarded: true }})
            break
		   case 'forwardaudio':                 
	                encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo	
                 	media = await pikachubo.downloadAndSaveMediaMessage(encmedia)
	                	...
	                	if (err) return reply('Error!')
		                hah = fs.readFileSync(ran)
	                	pikachubo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek, sendEphemeral: true, contextInfo: { forwardingScore: 508, isForwarded: true }})
	                	fs.unlinkSync(ran)
   	                 })
   	                	break
            ...
            if (err) return reply('Error!')
            buffer453 = fs.readFileSync(ran)
            pikachubo.sendMessage(from, buffer453, video, {mimetype: 'video/mp4', quoted: mek, sendEphemeral: true, contextInfo: { forwardingScore: 508, isForwarded: true }})
            fs.unlinkSync(ran)
            })
            break
          case 'fw':
pikachubo.sendMessage(from, `${args.join(' ')}`, MessageType.text, {contextInfo: { forwardingScore: 210, isForwarded: true }})
            break

//------------------< Sticker Maker >-------------------

case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} pikachubo`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              pikachubo.sendMessage(from, buffer, sticker, { quoted: mek })
              break
       case 'sticker':
       case 'stiker':
       case 's':
       ...
       case 'sgif':
              ...
              mediat = await pikachubo.downloadAndSaveMediaMessage(encmediat)
              ron = getRandom('.webp')
              ...
              if (error) return reply(`${error}`)
              pikachubo.sendMessage(from, fs.readFileSync(ron), sticker, {quoted:mek})
              fs.unlinkSync(ron)
})
})
              } else if ((isMedia && mek.message.videoMessage.seconds < 11 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
              encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
              mediat = await pikachubo.downloadAndSaveMediaMessage(encmedia)
              ron = getRandom('.webp')
              exec(`ffmpeg -i ${mediat} -vf "scale=512:512:force_original_aspect_ratio=increase,fps=15, crop=512:512" ${ron}`, (err) => {
              fs.unlinkSync(mediat)
              if (err) return reply(`${err}`)
...
//------------------< Storage >-------------------
	 ...
	 case 'delvn':
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_🍁_`
					pikachubo.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}𝒑𝒆𝒑𝒆 𝒔𝒆𝒓${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://i.ibb.co/vkkcm0L/034c588fd8d5.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/919633687665'}`}},quoted: ftext})
					break
				case 'addimage':
				    if (!isOwner) return
					...
					pikachubo.sendMessage(from, `*Image Added*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
					try {
					 nmm = body.slice(10)
				...
				case 'listimage':
					...
					pikachubo.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
              case 'addsticker':
                    if (!isOwner) return
					if (!isQuotedSticker) return reply('*Reply To Sticker*')
					...
					pikachubo.sendMessage(from, `*Sticker Added*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
					try {
					 nmm = body.slice(12)
...
//------------------< Tag >-------------------
       ...
       case 'tag':
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					pikachubo.sendMessage(from, tagq, text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
...
//------------------< Group >-------------------
       ...
       case 'block':
				 pikachubo.updatePresence(from, Presence.composing) 
				 pikachubo.chatRead (from)
					if (!isGroup) return reply(mess.group())
					if (!isOwner) return reply(mess.owner)
					pikachubo.blockUser (`${body.slice(7)}@c.us`, "add")
					pikachubo.sendMessage(from, `*ʙʟᴏᴄᴋᴇᴅ* ${body.slice(7)}@c.us`, text)
					break
		case 'unblock':
					if (!isGroup) return reply(mess.group)
					if (!isOwner) return reply(mess.owner)
				    pikachubo.blockUser (`${body.slice(9)}@c.us`, "remove")
					pikachubo.sendMessage(from, `*ᴜɴʙʟᴏᴄᴋᴇᴅ* ${body.slice(9)}@c.us`, text)
					break
         case 'getpic':
					if (!isGroup) return reply(mess.only.group)
            mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid[0]
            pictt = await pikachubo.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            pikachubo.sendMessage(from, pict, image, {quoted: mek})
            break
        case 'getpp':
               if (mek.message.extendedTextMessage === null || mek.message.extendedTextMessage === undefined) {
               linkpp = await pikachubo.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               pikachubo.sendMessage(from, buffer, image, {caption: "Nih", quoted: mek })
               } else if (mek.message.extendedTextMessage.contextInfo.mentionedJid === null || mek.message.extendedTextMessage.contextInfo.mentionedJid === undefined) {
               mberr = mek.message.extendedTextMessage.contextInfo.participant
               linkpp = await pikachubo.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               pikachubo.sendMessage(from, buffer, image, { quoted: mek, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
...
//------------------< Maker Menu >-------------------
                ...
                case 'thunder':
                    ...
                    pikachubo.sendMessage(from, ini_buffer, image, { quoted: ftroli})
                    break
                case 'pornhub':
                case 'glitch':
                case 'avenger':
		...
		if (!isRegistered) return reply(ind.noregis())
               cf = `${body.slice(8)}`
                ...
                case 'carvedwood':
                    ...
                    pikachubo.sendMessage(from, ini_buffer, image, { quoted: ftext})
                    break
                case 'tiktok':
                case 'arcade8bit':
                case 'battlefield4':
                case 'pubg':
                    ...
                    pikachubo.sendMessage(from, ini_buffer, image, { quoted: fgc})
                    break
...
let qse = pikachubo.serializeM(await m.getQuotedObj())
if (!qse.quoted) return reply('the message you replied does not contain a reply!')
await qse.quoted.copyNForward(m.chat, true)
break
     case 'groplist':
                case 'grouplist':
  const txs = pikachubo.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${pikachubo.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
...
if (!Number(oi2)) return reply('*The number must be a number!*')
	  for (let i = 0; i < oi2; i++) {
	  pikachubo.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('*Most!*')
	  ...
	  pikachubo.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await pikachubo.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('*The number must be a number!*')
	if (Number(args[0]) >= 50) return reply('*Most!*')
	  for (let i = 0; i < args[0]; i++) {
	  pikachubo.sendMessage(from, anu, sticker)
	  }
	...
	if (Number(args[0]) >= 50) return reply('*Most!*')
	  for (let i = 0; i < args[0]; i++) {
	  pikachubo.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
if (Number(oi2) >= 50) return reply('*Most!*')
	if (!Number(oi2)) return reply('*The number must be a number!*')
	  for (let i = 0; i < oi2; i++) {
	  pikachubo.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
      case 'delchat':
                ...
                await sleep(4000)
                pikachubo.modifyChat(from, ChatModification.delete)
                break
     case 'mute':
			    if (!isOwner && !mek.key.fromMe) return reply(mess.only.owner)
			    if (!isGroup) return reply(mess.only.group)
                ...
                break
     case 'tts':
					if (args.length < 1) return pikachubo.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return pikachubo.sendMessage(from, `Teksnya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
					...
					bby.length > 300
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								pikachubo.sendMessage(from, buff, audio, { duration: 359996400, ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
        case 'demote':
		    ...
		    pikachubo.groupDemoteAdmin(from, [demote])
		    ...
		    pikachubo.groupMakeAdmin(from, [promote])
						reply('*Successful Promoted an Admin')
						break
                case 'linkgc':
				if (!isGroup) return reply(mess.only.group)
					linkgc = await pikachubo.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					pikachubo.sendMessage(from, yeh, text, { quoted: fgc })
					break
     case 'resetlinkgroup':
         case 'revoke':
         if (!isGroup) return reply(mess.only.group)
         ...
         pikachubo.query({json, expect200: true})
          reply('*Succes Reset Group Link*')
         break
     case 'tagme':
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@')[0]}`
					pikachubo.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
                    case 'fdeface':
                              var nn = body.slice(9)
				...
				var bot = gh.split("|")[2];
			    pikachubo.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
                case 'return':
				if (!isOwner) return 
					return pikachubo.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return pikachubo.sendMessage(from, `root @Denis Ser:~ ${err}`, text, { quoted: mek })
                 break
              case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							ppp = `${args.join(' ')}`
							...
							const media = await pikachubo.downloadAndSaveMediaMessage(encmediao, `./sticker/${sender}`)
							...
							await ffmpeg(`${media}`)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/stickwm_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											pikachubo.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
							...
							const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await pikachubo.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
...
if (!isOwner) return
try {
return pikachubo.sendMessage(from, 
...
exec(exc, (err, stdout) => {
if (err) return pikachubo.sendMessage(from, `root @denis:~ ${err}`, text, { quoted: mek })
if (stdout) {
pikachubo.sendMessage(from, stdout, text)
}
})
}
if (budy.startsWith('>')) {
